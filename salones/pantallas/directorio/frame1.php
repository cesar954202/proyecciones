...
if ($result = $mysqli->query("SELECT COUNT(DISTINCT(eventos.id_evento)) as 'numero' from programacion_salones 
        ...
        if ($result->num_rows > 0){
          while ($row = $result->fetch_object()){
            $numero=$row->numero;
            if($numero>6){
              echo "<META HTTP-EQUIV=".'REFRESH'." CONTENT=".'15;URL=frame2.php'.">";
            }else{
...
if ($result = $mysqli->query("SELECT DISTINCT(eventos.id_evento) as 'evento' from programacion_salones  
                              ...
                              WHERE programacion_salones.fecha_fin >= (SELECT CURRENT_TIMESTAMP) 
                              AND fecha_inicio <= (select DATE_SUB(CURDATE(), INTERVAL -1 DAY)) ORDER BY fecha_inicio limit 0,6")){
        if ($result->num_rows > 0){
          while ($row = $result->fetch_object()){
            $evento=$row->evento;
            //echo "evento: ".$evento;
            echo "<iframe  class=".'w3-card-4'." style=".'display:block'." scrolling=".'no'." frameBorder=".'0'." width=".'100%'." height=".'220'." class=".'w3-center'." src="."evento.php?id_evento=". urlencode($evento) ."></iframe><br><br>";
