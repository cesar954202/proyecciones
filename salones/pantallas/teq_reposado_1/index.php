...
<header class="w3-container w3-plata w3-bottombar w3-border-amber w3-card-4 " style=" height: 640px " >
  ...
  <div class="w3-col w3-third " style="height: 398px">
    <div class="w3-panel">
      <h2 class="w3-text-white w3-center">Eventos del d&iacute;a</h2><br>
      <h2 class="w3-text-white w3-center w3-border-amber w3-bottombar w3-topbar">Sal&oacute;n Tequila reposado</h2>
    </div>
  </div>
...
//carga datos
      if ($result = $mysqli->query("SELECT programacion_salones.id_evento,programacion_salones.completo from programacion_salones WHERE (SELECT CURRENT_TIMESTAMP)>= programacion_salones.fecha_transmision AND programacion_salones.fecha_fin >= (SELECT CURRENT_TIMESTAMP) AND (programacion_salones.id_pantalla=13 OR programacion_salones.id_pantalla=14)")){
        if ($result->num_rows > 0){
          if($result->num_rows == 2){
            echo "<iframe src=".'evento_corto.php'."?pantalla=".'13'." style=".'overflow:hidden;'." scrolling=".'no'." frameBorder=".'0'." width=".'100%'." height=".'620'." class=".'w3-center'."></iframe><br>";
            echo "<iframe src=".'evento_corto.php'."?pantalla=".'14'." style=".'overflow:hidden;'." scrolling=".'no'." frameBorder=".'0'." width=".'100%'." height=".'620'." class=".'w3-center'."></iframe>";
          }else{
