...
<?php
//carga datos
      if ($result = $mysqli->query("SELECT programacion_salones.completo from programacion_salones WHERE programacion_salones.fecha_transmision < (SELECT CURRENT_TIMESTAMP) AND programacion_salones.id_pantalla=1  AND programacion_salones.fecha_fin > (SELECT CURRENT_TIMESTAMP)")){
